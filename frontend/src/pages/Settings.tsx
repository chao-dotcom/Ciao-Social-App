...
const Settings: React.FC = () => {
  ...
  const [linkedAccounts, setLinkedAccounts] = useState<OAuthProvider[]>([]);
  
  useEffect(() => {
    // Load linked OAuth accounts
    if (user && (user as any).authProviders) {
      setLinkedAccounts((user as any).authProviders);
    }
  }, [user]);
  ...
  const handleUnlinkProvider = async (provider: string) => {
    ...
    try {
      ...
      setError((err as Error).message || `Failed to unlink ${provider} account`);
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="space-y-8">
            ...
            <section>
              ...
              <div className="bg-gray-50 rounded p-4 space-y-3">
                ...
                <div className="flex justify-between text-sm text-gray-700"><span className="font-semibold">Member since</span><span className="text-gray-900">{user?.createdAt ? new Date(user.createdAt).toLocaleDateString() : 'N/A'}</span></div>
              </div>
            </section>

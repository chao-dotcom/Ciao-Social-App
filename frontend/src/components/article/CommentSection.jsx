              ...
              <div className="bg-gray-50 p-3 rounded">
                ...
                {editingId === comment._id ? (
                  <div className="mt-2 flex gap-2">
                    <input type="text" value={editText} onChange={(e) => setEditText(e.target.value)} className="flex-1 border rounded px-2" />
...
const CommentSection = ({ articleId, commentsCount }) => {
  const [comments, setComments] = useState([]);
  const [newComment, setNewComment] = useState('');
  const [loading, setLoading] = useState(false);
  const [editingId, setEditingId] = useState(null);
        ...
        <div className="mt-3">
          ...
          <div className="mt-3 space-y-3">
            {comments.map(comment => (
              <div key={comment._id} className="flex items-start gap-3">
                <Link to={`/profile/${comment.author}`}>
                  <img src={comment.authorDetails?.avatar} alt={comment.author} className="w-8 h-8 rounded-full object-cover" />
                </Link>
